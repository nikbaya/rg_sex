...
def annotate_phen(tb, phen, sex, phen_tb_dict, filter_to_phen=True):
    ...
    if phen_tb.phen.dtype == hl.dtype('bool'):
        tb2 = annotate_fn(self=tb1, phen = hl.bool(tb1.phen_str)).drop('phen_str')
    else:
        tb2 = annotate_fn(self=tb1, phen = hl.float64(tb1.phen_str)).drop('phen_str')
...
def prs(mt, phen, sex, n_remove, prune, percentiles, seed):
    r'''
...
#                '2217_raw':['Age started wearing contact lenses or glasses (raw)', 73178],
    ...
    use_sex_spec_irnt = False # whether to use sex-specific phenotype information when comparing against PRS
    ...
    #            except:
        ...
        if calc_prs:
            for sex in ['female','male','both_sexes']:
                mt_sex0 = get_test_mt(mt_all=mt0, sex='both_sexes', seed=phen_dict[phen][1])

                mt_sex = annotate_phen(tb=mt_sex0, phen=phen, sex='both_sexes', phen_tb_dict=phen_tb_dict) #use the both sexes irnt
                
                prs(mt=mt_sex, phen=phen, sex=sex, n_remove=n_remove_per_sex,
            ...
            for sex in ['female','male','both_sexes']:
                ...
                if use_sex_spec_irnt and sex!='both_sexes' and 'irnt' in phen:
                    ...
                    mt_sex3 = annotate_phen(tb=mt_sex2, phen=phen, sex='male', 
                                           phen_tb_dict=phen_tb_dict, filter_to_phen=False)
                    mt_sex2 = mt_sex1.rename({'phen':'phen_male'})
                    mt_sex = mt_sex2.annotate_col(phen = hl.is_defined(mt_sex2.phen_female)*mt_sex2.phen_female +
                                                 hl.is_defined(mt_sex2.phen_male)*mt_sex2.phen_male)
