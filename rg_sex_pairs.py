...
phen_summary = 'gs://nbaya/split/test/height.split.tsv'
...
parser.add_argument('--phenfile', type=str, required=True, help="tsv of phenotypes with sample sizes, in working directory (brittle spec)")
...
rg_outname = 'ukbb31063.phesant.rg_sex.batch_'+str(args.paridx)+'.tsv.gz'
#
########
...
# copy phenfile to local wd (Added by Nik)
#test_path = 'gs://nbaya/h2part/ukbb31063.both_sexes.h2part_results.phesant.tsv.gz'
#subprocess.call(['gsutil','cp',gs_phenfile_bucket+args.phenfile,'file://',wd])
#gs://ukbb-gwas-imputed-v3-results/export2/phenotypes.{}.tsv.bgz

gs_phenfile_bucket = 'gs://nbaya/rg_sex'
gs_ss_path1 = gs_phenfile_bucket + '/' + args.phenfile
loc_path1 = wd + '/'
...
# read phenotype file
print wd+'/'+args.phenfile
phens = pd.read_table(wd+'/'+args.phenfile)
